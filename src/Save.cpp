...
void MatrixToFloatArray(glm::mat4 &Mat, float * FloatArray)
{
    for(int i=0; i<16; i++)
    {
        int col = i % 4; int row = i / 4;
        FloatArray[i] = Mat[row][col];
    }
}

void FloatArrayToMatrix(glm::mat4 &Mat, float * FloatArray)
{
    for(int i=0; i<16; i++)
    {
        int col = i % 4; int row = i / 4;
        Mat[row][col] = FloatArray[i];
    }
}
        ...
        if(CurrentSceneItemIndex != UINT32_MAX)
        {
            ...
            if(ImGui::Begin(Scene->InstancesPointers[CurrentSceneItemIndex]->Name.c_str(), nullptr, ImGuiWindowFlags_NoSavedSettings))
            {
                ...
                if(ImGui::CollapsingHeader("Transform"))
                {
                    ...
                    // Hack : For some reason, glm::value_ptr doesn't really work on matrices ? Returns only the first element
                    MatrixToFloatArray(Scene->InstancesPointers[CurrentSceneItemIndex]->InstanceData.Transform, &Matrix[0]);
                    ImGuizmo::DecomposeMatrixToComponents(&Matrix[0], &Translation[0], &Rotation[0], &Scale[0]);
                    UpdateTransform |= ImGui::DragFloat3("Position", &Translation[0], 0.01f);
                    UpdateTransform |= ImGui::DragFloat3("Rotation", &Rotation[0], 0.01f);
                    UpdateTransform |= ImGui::DragFloat3("Scale", &Scale[0], 0.01f);
                    ImGuizmo::RecomposeMatrixFromComponents(&Matrix[0], &Translation[0], &Rotation[0], &Scale[0]);
                    FloatArrayToMatrix(Scene->InstancesPointers[CurrentSceneItemIndex]->InstanceData.Transform, &Matrix[0]);
                }
                ...
                if(Renderers[CurrentRenderer]->UseGizmo)
                {
                    ...
                    static float ModelArray[16];
                    static float ViewArray[16];
                    ...
                    MatrixToFloatArray(Scene->Camera.GetViewMatrix(), &ViewArray[0]);
                    MatrixToFloatArray(projectionMatrix, &ProjectionArray[0]);
                    MatrixToFloatArray(CorrectedTransform, &ModelArray[0]);
                    
                    // UpdateTransform |= ImGuizmo::Manipulate(&ViewArray[0], 
                    //                     &ProjectionArray[0], 
                    ...
                    // FloatArrayToMatrix(CorrectedTransform, &ModelArray[0]);
